...
void BuildNIterativeCuts(TGraph *geff,double *N_toplot,double *Eline,int const & Nlines,string const & scale);
int main(int argc, char *argv[])
{
	gStyle->SetOptStat(0);	
	//TApplication *toto=new TApplication("toto",0,0);
	std::vector<string> vec_Directories;	
	vec_Directories.push_back("ProcessedData");	
	const int Ndirectories=(int)vec_Directories.size();
	string masterdirectory="";
	string masterpath=Form("%s",masterdirectory.c_str());	
	string DATA_Path="ProcessedData";
...
//-------------------------------------------------------------------------------------------------------------
	...
	int count_line_mc = 0 ;
	vecruns.push_back(Runs_to_analyse);
   ...
   string template_2 = "Slow";
	...
	string rootfilesaveLines="Bank_Lines/"+study_index+"_"+BankLineNameBegin+"_"+common_name+"_Bank_Lines.root";	
	if(SIMU_Type!="Line"){saveLines=false;}
	if(verbose > 0) cout<<rootfilesaveLines<<endl;		
	if(SIMU_Type=="Line"){	
///////////////// LOOK FOR LINES ////////
	   string commandline="ls -d Simulated_data/SimuCoinc/Line* | awk -F\"/Line_\" '{print $2}' | awk -F\"_\" '{print $1}' | sort -n | uniq > templist_Lines.txt";	
...
/////////////////////////LOOP TOLERANCE////////////////////////////////////////////////////////////////////////
   ...
   if(det=="NbSi209") triggerchanel=1;
	for(int dir=0;dir<Ndirectories;dir++){	
	   ...
	   string SIMUCOINC_Path="Simulated_data/SimuCoinc";
	   if(verbose > 0) cout<<"before loop over runs"<<endl;
	   if(verbose > 0) cout<<SIMUCOINC_Path<<" Nrun "<<Nbruns<<endl;
	   for(int r=0;r<Nbruns;r++){	
...
// determine the number of iterations			
			   ...
			   for(int t=0;t<NTYPES;t++){
				   ...
				   if(SIMU_Type=="Flat"){
				      ...
				      pattern=Form("%s/%s_%s_*",SIMUCOINC_Path.c_str(),SIMU_Type.c_str(),vec_Lines[t].c_str());
				   }else{
				   ...
				   for (int SIMU_iteration=1;SIMU_iteration<=SIMU_NTOT_iterations;SIMU_iteration++){
					...
					//string SIMU_File=Form("ProcessedData_%s_S*[02468]_%s_%s_ChanTrig%i.root",run.c_str(),format.c_str(),filenumber,det.c_str(),SIMU_condition.c_str(),triggerchanel);	
				   	...
				   	string SIMUCOINC_RELATIVE_Path=Form("%s/%s_%s_%d",SIMUCOINC_Path.c_str(),SIMU_Type.c_str(),vec_Lines[t].c_str(),SIMU_iteration);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
						   ...
						   if(Energy_OFInput[0]>=EminBank && Energy_OFInput[0]<=EmaxBank && Energy_OFInput[1]>=EminBank && Energy_OFInput[1]<=EmaxBank){
				 		   ...
				 		   if(energy_in_kev>0){
				 		      if(Normal_chi2normalizedIonA <= 1.8 && Normal_chi2normalizedIonB <= 1.8){
...
TGraph       *eV_N_cut_RT_Maintenance      =new TGraph();
TGraph       *eV_N_cut_Bank                =new TGraph();
TGraph       *eV_N_cut_Trigger             =new TGraph();
TGraph       *eV_N_cut_Chi2NormION         =new TGraph();
TGraph       *eV_N_cut_Eion                =new TGraph();
TGraph       *eV_N_cut_chalfid             =new TGraph();
TGraph       *eV_N_cut_Chi2BNorm           =new TGraph();
TGraph       *eV_N_cut_TOT                 =new TGraph();
BuildNIterativeCuts(eV_N_cut_RT_Maintenance,N_cut_RT_Maintenance,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_Bank,N_cut_Bank,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_Trigger,N_Trigger,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_Chi2NormION,N_cut_Chi2NormIon,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_Eion,N_cut_Eionization,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_chalfid,Ncut_FidChal,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_Chi2BNorm,N_cut_Chi2NormB,Eline,Nlines,"eV");
BuildNIterativeCuts(eV_N_cut_TOT,N_all,Eline,Nlines,"eV");
...
eV_geff_cut_Chi2BNorm           ->Write("Efficienciy_cutChihB");
eV_geff_cut_Chi2Norm            ->Write("Efficienciy_cutChihA");
eV_N_cut_RT_Maintenance         ->Write("Nevent_cutRTMTN");
eV_N_cut_Bank                   ->Write("Nevent_cutBank");
eV_N_cut_Trigger                ->Write("Nevent_cutTrigger");
eV_N_cut_Chi2NormION            ->Write("Nevent_cutChiIon");
eV_N_cut_Eion                   ->Write("Nevent_cutEion");
eV_N_cut_chalfid                ->Write("Nevent_cutFid");
eV_N_cut_Chi2BNorm              ->Write("Nevent_cutChihB");
eV_N_cut_TOT                    ->Write("Nevent_Tot");
