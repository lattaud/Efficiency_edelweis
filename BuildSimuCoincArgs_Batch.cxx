...
int main(int argc, char *argv[])
{
	...
	string DATA_Path="ProcessedData"; // repertoire données
	string SIMU_Path="Simulated_data/Simu"; // repertoire SIMU
	...
	for(int r=0;r<Nbruns;r++){
		string commandline=Form("ls -d %s/Line_*_* | awk -F\"/Line_\" '{print $2}' | awk -F\"_\" '{print $1}' | sort -n | uniq > templist_%s_Lines.txt",SIMU_Path.c_str(),vecruns[r].c_str());
	...
	string commandline=Form("ls -d %s/Line_*_* | awk -F\"/Line_\" '{print $2}' | awk -F\"_\" '{print $1}' | sort -n | uniq > templist_Lines.txt",SIMU_Path.c_str());
	cout<<commandline<<endl;
	system(commandline.c_str());
	ifstream fline;
		...
		for(int filenumber=0;filenumber<nfiles;filenumber++){
		   ...
		   string DATA_PathAndFile=Form("%s/%s/%s/%s",DATA_Path.c_str(),run.c_str(),det.c_str(),DATA_File.c_str());
			...
			for(int t=0;t<NTYPES;t++){
				string pattern;
				if(SIMU_Type=="Flat"){
					pattern=Form("%s/%s_%.4f_%.4f_*",SIMU_Path.c_str(),SIMU_Type.c_str(),SIMU_Emin,SIMU_Emax);
				}else if(SIMU_Type=="Line"){
					pattern=Form("%s/%s_%s_*",SIMU_Path.c_str(),SIMU_Type.c_str(),vec_Lines[t].c_str());
				...
				if(t%100 == 0)cout<<SIMU_NTOT_iterations<<" iterations for "<<SIMU_Type.c_str()<<" "<<vec_Lines[t].c_str()<<endl;
				
				
				for (int SIMU_iteration=1;SIMU_iteration<=SIMU_NTOT_iterations;SIMU_iteration++){
					
					if(t%100 == 0) cout<<Form("run %d/%d , file %d/%d , Type %d/%d , iteration %d/%d",r,Nbruns,filenumber,nfiles,t,NTYPES,SIMU_iteration,SIMU_NTOT_iterations)<<endl;
					   ...
					   string SIMU_RELATIVE_Path=Form("%s/%s",SIMU_Path.c_str(),SIMU_condition.c_str());
					...
					//cout<<"simu : "<<SIMU_EVEREST_PathAndFile.c_str()<<endl;
				   	...
				   	string SIMUCOINC_RELATIVE_Path=Form("%s/%s",SIMUCOINC_Path.c_str(),SIMU_condition.c_str());
			 	...
			 	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
                  ...
                  if(t%100 == 0)cout<<"before"<<endl;
                  if(t%100 == 0)cout<<"n events in EvenTree_trig_Normal ="<<EventTree_trig_Normal_filt->GetEntries()<<endl;
                  bool empty=false; 
                  ...
                  if((int)filesempty.size()>=1){
                     ...
                     if(t%100 == 0)cout<<"non empty"<<endl;
                  ...
                  tcoinc->Fill();
				   	...
				   	if(t%100 == 0)cout<<"out of loop over "<<Nentries_SimuOnly<<" simu entries"<<endl;
				   	fcoinc->cd();
				   	tcoinc->Write("",TObject::kOverwrite);
				   	fcoinc->Close();
               ...
               if(t%100 == 0)cout<<"out of loop over "<<SIMU_NTOT_iterations<<" simu iterations"<<endl;
