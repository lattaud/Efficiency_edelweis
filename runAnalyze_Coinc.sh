...
if [ "$jobTyp" == "Batch" ]; then 
   for Det in "${Detectors[@]}"; do
     ...
     echo "jobName"
     echo $jobName     
	 mkdir -p  $SOFT_DIR/logFiles
	 chmod -R g+rw $SOFT_DIR/logFiles
	 rm -f $SOFT_DIR/logFiles/$jobName.log
         qsub -v run,detector,trigchan  \
          -v SOFT_DIR,DATA_DIR,Coinc_DIR,jobName  \
          -v ProcessedData_Dir,SimuData_Dir,Tension,Ion_cut \
	      -N $jobName -P P_edelweis -l vmem=4G -l sps=1 -j y  \
	      -o $SOFT_DIR/logFiles/$jobName.log runAnalyze_Coinc.csh
   done
else
    source runAnalyze_Coinc.csh run detector trigchan SOFT_DIR DATA_DIR OUT_DIR jobName ProcessedData_Dir SimuData_Dir Tension Ion_cut
fi 
#################<---- Boucle Batch qsub
